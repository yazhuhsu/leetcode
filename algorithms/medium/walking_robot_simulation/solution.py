class Solution:
    def robotSim(self, commands: list, obstacles: list) -> int:
        ...
        for _, obstacle in enumerate(obstacles):
            if obstacle[0] not in point_dict:
                point_dict[obstacle[0]] = []
            point_dict[obstacle[0]].append(obstacle[1])
        ...
        for _, command in enumerate(commands):
            ...
            if directions[direction] == 'U':
                for idx in range(command):
                    y += 1
                    if x in point_dict and y in point_dict[x]:
                        y -= 1
                        break
...
# Edge cases
print(solution.robotSim([], []) == 0)            # empty commands
print(solution.robotSim([-1,-1,-2], []) == 0)    # turn commands only
